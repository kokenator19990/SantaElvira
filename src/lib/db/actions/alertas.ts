...
import { getPeriodoActualId } from "../queries/flota";
...
export async function resolverAlerta(
  ...
  try {
    ...
    return { ok: false, error: errorSeguro(e, "alertas") };
  }
}
...
export async function resolverTodasPorEstado(
  ...
  // C3: limitar la resolución en lote al período activo — evita resolver alertas históricas
  const periodoId = await getPeriodoActualId();
  if (periodoId === null) return { ok: false, error: "No hay período activo" };
  ...
  try {
    const resultado = await db
      ...
      .where(and(eq(t.alerta.estado, estado), eq(t.alerta.resuelta, false), eq(t.alerta.periodoId, periodoId)))
      .returning({ id: t.alerta.id });
