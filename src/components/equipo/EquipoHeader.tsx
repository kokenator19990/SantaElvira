...
export function EquipoHeader({ equipo }: EquipoHeaderProps) {
  return (
    <div className="flex flex-col gap-3">
      <Link
        href="/flota"
        className="flex items-center gap-1 text-[12px] text-[#A1A1AA] hover:text-[#52525B] transition-colors w-fit"
      >
        <ChevronLeft size={13} />
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            <div className="flex items-center gap-2.5 flex-wrap">
              <h1 className="text-[25px] font-mono font-bold text-[#09090B]">{equipo.id}</h1>
              <StatusBadge estado={equipo.semaforo.general} />
            </div>
            <p className="text-[15px] text-[#71717A] mt-0.5">{equipo.modelo}</p>
            {equipo.paroTotal && (
              <div className="flex items-center gap-1.5 mt-2 px-2.5 py-1.5 rounded-[6px] bg-[#FEF2F2] border border-[#FECACA] w-fit">
                ...
                <p className="text-[13px] text-[#991B1B] font-medium">{equipo.motivoParo}</p>
              </div>
            )}
          </div>
        </div>

        <div className="flex gap-4 text-[13px] text-[#71717A]">
