...
export function KpisAdminClient({ flota, periodos }: { flota: Equipo[]; periodos: Periodo[] }) {
  ...
  async function guardarFila(idx: number) {
    ...
    if (!resKpi.ok)    return patch(idx, { status: "error", errorMsg: resKpi.error });
    if (!resAsarco.ok) return patch(idx, { status: "error", errorMsg: resAsarco.error });
    patch(idx, { status: "ok" });
  }
  ...
  async function guardarTodos() {
    ...
    for (let i = 0; i < rows.length; i++) {
      await guardarFila(i);
      const after = rows[i];
      if (after.status === "error") errores.push(`${after.equipoId}: ${after.errorMsg}`);
    }
    if (errores.length > 0) {
      setGlobalMsg({ type: "error", text: `${errores.length} errores. Primero: ${errores[0]}` });
    } else {
      setGlobalMsg({ type: "ok", text: `${rows.length} equipos guardados.` });
    }
  }
  ...
  function regenerarAlertas() {
    ...
    startTransition(async () => {
      ...
      else      setGlobalMsg({ type: "error", text: r.error });
    });
  }
    ...
    <div className="flex flex-col gap-5 max-w-[1400px] mx-auto">
      ...
      <div className="overflow-x-auto rounded-[10px] border border-[#E4E4E7] bg-white">
        <table className="w-full text-[13px]">
          ...
          <tbody>
            {rows.map((r, idx) => {
              const sumAsarco = r.pctOperativo + r.pctReserva + r.pctDetProgramada + r.pctDetNoProg + r.pctPerdidaOp;
              const sumOk = Math.abs(sumAsarco - 100) <= 0.5;
              return (
                <tr key={r.equipoId} className="border-t border-[#F4F4F5] hover:bg-[#FAFAFA]">
                  ...
                  <Num value={r.tiempoOperativo} onChange={(v) => patch(idx, { tiempoOperativo: v })} />
                  <Num value={r.reserva} onChange={(v) => patch(idx, { reserva: v })} />
                  <Num value={r.horasAcumuladas} onChange={(v) => patch(idx, { horasAcumuladas: v })} step={1} />
                  <td className="px-1 py-1.5 text-center">
                    <input
                      type="checkbox"
                      checked={r.paroTotal}
                      onChange={(e) => patch(idx, { paroTotal: e.target.checked })}
                      className="cursor-pointer"
                      title={r.paroTotal ? r.motivoParo : "Marcar paro total"}
                    />
                  </td>
                  <Num value={r.pctOperativo}     onChange={(v) => patch(idx, { pctOperativo: v })} />
                  <Num value={r.pctReserva}       onChange={(v) => patch(idx, { pctReserva: v })} />
